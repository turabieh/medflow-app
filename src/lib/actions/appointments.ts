...
export interface ConfirmBookingInput {
  ...
  // New name-parts system (migration 0043 — full_name is generated)
  firstName: string;
...
export async function confirmBooking(
  ...
  const nameToCheck = input.firstName?.trim() || input.fullName?.trim();
  if (!nameToCheck || !input.phone?.trim()) {
  ...
  const { error: patientError } = await supabase
    .from("patients")
    .update({
      // Use name parts (full_name is generated by DB from parts)
