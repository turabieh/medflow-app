...
function ServiceCard({ service: init, onSave, onDelete, isNew }: { service: R; onSave: (d:R)=>void; onDelete?: ()=>void; isNew?: boolean }) {
  ...
  return (
    <div className="rounded-xl border-2 border-neutral-900 bg-white p-4 shadow-sm space-y-3">
      ...
      <div>
        <label className="mb-2 block text-[10px] font-semibold text-neutral-600 uppercase tracking-wide">Service Image</label>
        <ImageUploadButton
        ...
        />
        <input value={previewImg} onChange={e => setS({...s, image_url: e.target.value})}
          className={`${inp} mt-2`} placeholder="or paste image URL..." />
      </div>

      {/* Names */}
      <div className="grid grid-cols-2 gap-3">
        <div><label className="mb-1 block text-[10px] text-neutral-500">Name (EN) *</label>
          <input value={String(s.name_en ?? "")} onChange={e => setS({...s, name_en: e.target.value})} className={inp} placeholder="e.g. Neurology" /></div>
        <div><label className="mb-1 block text-[10px] text-neutral-500">Name (AR) *</label>
          <input value={String(s.name_ar ?? "")} onChange={e => setS({...s, name_ar: e.target.value})} className={inp} dir="rtl" placeholder="مثال: طب الأعصاب" /></div>
      </div>

      {/* Descriptions */}
      <div className="grid grid-cols-2 gap-3">
        <div><label className="mb-1 block text-[10px] text-neutral-500">Description (EN)</label>
          <textarea value={String(s.description_en ?? "")} onChange={e => setS({...s, description_en: e.target.value})} rows={2} className={`${inp} resize-none`} /></div>
        <div><label className="mb-1 block text-[10px] text-neutral-500">Description (AR)</label>
          <textarea value={String(s.description_ar ?? "")} onChange={e => setS({...s, description_ar: e.target.value})} rows={2} className={`${inp} resize-none`} dir="rtl" /></div>
      </div>
