        ...
        <div>
          <div className="mb-5 flex flex-wrap items-end gap-3">
            <div><label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-neutral-500">From</label>
              <input type="date" value={cashFrom} onChange={e=>setCashFrom(e.target.value)} className="rounded-md border border-neutral-300 px-3 py-2 text-sm outline-none" /></div>
            <div><label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-neutral-500">To</label>
              <input type="date" value={cashTo} onChange={e=>setCashTo(e.target.value)} className="rounded-md border border-neutral-300 px-3 py-2 text-sm outline-none" /></div>
            <button onClick={async()=>{
              ...
              const {data:appts}=await sb.from("appointments")
                ...
                .eq("clinic_id",clinicId).eq("payment_method","cash").eq("payment_confirmed",true)
                .gte("appt_date",cashFrom).lte("appt_date",cashTo).order("appt_date",{ascending:false});
              ...
              setCashList((appts??[]).map((a:any)=>({id:a.id,appt_date:a.appt_date,payment_amount:a.payment_amount,patientName:pm[a.patient_id]??"Unknown",doctorName:Array.isArray(a.users)?a.users[0]?.full_name??"—":a.users?.full_name??"—"})));
              setCashLoading(false);
            }} disabled={cashLoading} className="rounded-md bg-neutral-900 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">
          ...
          <div className="mb-5 flex gap-4 flex-wrap">
            <div className="rounded-xl border border-neutral-200 bg-white px-5 py-4 shadow-sm">
              ...
              <p className="mt-1 text-2xl font-bold">{cashList.reduce((s,p)=>s+(p.payment_amount??0),0).toFixed(2)} <span className="text-sm font-normal text-neutral-400">{currency}</span></p>
            </div>
            <div className="rounded-xl border border-neutral-200 bg-white px-5 py-4 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-wide text-neutral-400">Transactions</p>
              <p className="mt-1 text-2xl font-bold">{cashList.length}</p>
            </div>
          </div>
          {cashList.length===0?(
            <div className="rounded-xl border border-dashed border-neutral-300 py-12 text-center text-neutral-400">Select a date range and click Apply.</div>
          ):(
                  ...
                  {cashList.map(p=>(
                    <tr key={p.id} className="hover:bg-neutral-50">
                      ...
                      <td className="px-4 py-3 text-right">
                        {editCashId===p.id
                          ?<input type="number" value={editCashAmt} onChange={e=>setEditCashAmt(e.target.value)} min={0} step={0.01} autoFocus onKeyDown={e=>{if(e.key==="Escape")setEditCashId(null);}} className="w-28 rounded-md border border-blue-400 px-2 py-1 text-right text-sm font-semibold outline-none"/>
                          :<span className={`font-semibold ${cashSaved===p.id?"text-green-600":"text-neutral-900"}`}>{cashSaved===p.id?"✓ ":""}{(p.payment_amount??0).toFixed(2)}</span>
                        }
                      </td>
                      <td className="px-4 py-3 text-center">
                        {editCashId===p.id
                          ?<div className="flex items-center justify-center gap-2">
                            ...
                            }} className="rounded-md bg-neutral-900 px-3 py-1 text-xs font-semibold text-white disabled:opacity-50">{cashSaving?"...":"Save"}</button>
                            <button onClick={()=>setEditCashId(null)} className="rounded-md border border-neutral-300 px-3 py-1 text-xs text-neutral-500">Cancel</button>
                          </div>
                          :<button onClick={()=>{setEditCashId(p.id);setEditCashAmt(String(p.payment_amount??""));}} className="rounded-md border border-neutral-200 px-3 py-1 text-xs font-medium text-neutral-600 hover:border-neutral-400">Edit</button>
                        }
                      </td>
                    </tr>
                  ))}
          ...
          <p className="mt-3 text-xs text-neutral-400">Click Apply to load payments. Edit to correct a wrong amount.</p>
        </div>
      )}
...
export function FinanceDashboard({
  ...
  const [cashList, setCashList] = useState<{id:string;appt_date:string;payment_amount:number|null;patientName:string;doctorName:string}[]>([]);
  ...
  const [cashFrom, setCashFrom] = useState(fromDate);
  const [cashTo, setCashTo] = useState(toDate);
