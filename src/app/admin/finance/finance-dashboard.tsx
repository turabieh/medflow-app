        ...
        <div>
          <div className="mb-5 flex flex-wrap items-end gap-3">
            <div>
              ...
              <input type="date" value={cashDateFrom} onChange={e=>setCashDateFrom(e.target.value)} className="rounded-md border border-neutral-300 px-3 py-2 text-sm outline-none"/>
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-neutral-500">To</label>
              <input type="date" value={cashDateTo} onChange={e=>setCashDateTo(e.target.value)} className="rounded-md border border-neutral-300 px-3 py-2 text-sm outline-none"/>
            </div>
            <button onClick={async()=>{
              ...
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              setCashRows((appts??[]).map((a:any)=>({id:a.id,appt_date:a.appt_date,payment_amount:a.payment_amount,patientName:pm[a.patient_id]??"Unknown",doctorName:Array.isArray(a.users)?a.users[0]?.full_name??"—":a.users?.full_name??"—"})));
          ...
          {cashRows.length===0?(
            <div className="rounded-xl border border-dashed border-neutral-300 py-12 text-center text-neutral-400">Select a date range and click Apply to load payments.</div>
          ):(
