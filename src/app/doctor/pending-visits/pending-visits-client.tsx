...
import { markDone } from "@/lib/actions/appointments";
...
export default function PendingVisitsClient({ appointments }: { appointments: Appt[] }) {
    ...
    return (
      <tr className={`hover:bg-neutral-50 ${isPast ? "bg-amber-50/40" : ""}`}>
        ...
        <td className="px-4 py-3 text-sm">
          <span className={isPast ? "font-semibold text-amber-700" : "text-neutral-600"}>{a.appt_date}</span>
        ...
        <td className="px-4 py-3 text-right">
          <div className="flex items-center justify-end gap-2">
            {a.visitId && (
              <Link href={`/doctor/visit/${a.visitId}`}
            ...
            {confirmId === a.id ? (
              <div className="flex items-center gap-1.5">
                <span className="text-xs text-neutral-600">Mark done?</span>
                <button disabled={markingDone === a.id} onClick={() => handleMarkDone(a.id)}
                  className="rounded bg-emerald-600 px-2 py-0.5 text-xs font-semibold text-white disabled:opacity-50">
                  {markingDone === a.id ? "..." : "Yes"}
                </button>
  ...
  return (
    <div className="p-6">
      <div className="mb-5">
        <h1 className="text-lg font-medium text-neutral-900">Patients Still With You</h1>
        <p className="text-sm text-neutral-500">
          {filtered.length} patient{filtered.length !== 1 ? "s" : ""} with status "with doctor" — mark done when finished
        </p>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl border border-dashed border-neutral-300 py-16 text-center text-neutral-400">
          ...
          <p className="text-sm font-medium">No pending visits — all done!</p>
        </div>
      ) : (
        <>
          {pastDue.length > 0 && (
