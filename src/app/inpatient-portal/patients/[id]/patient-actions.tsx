...
export function PortalPatientActions({
  inpatientId, patientId, hospitalId, doctorId, clinicId,
  todayVisitId, isActive, hospitalPatientId,
}: {
  inpatientId: string; patientId: string; hospitalId: string;
  doctorId: string; clinicId: string;
  todayVisitId?: string; isActive: boolean; hospitalPatientId: string;
}) {
  ...
  async function saveVisit() {
    ...
    const { data: visit, error: ve } = await supabase.from("visits").insert({
      ...
      inpatient_id: inpatientId, visit_date: today, visit_type: visitType,
      visit_fee: visitFee ? parseFloat(visitFee) : null,
      visit_context: "inpatient", status: "done",
      symptoms: visitNote || null,
    }).select("id").single();
    setSaving(false);
    if (ve || !visit) { setError(ve?.message ?? "Failed"); return; }
  ...
  async function saveLocation() {
    ...
    setSaving(true);
    const supabase = createClient();
    await supabase.from("inpatients").update({ location: location.trim() }).eq("id", inpatientId);
  ...
  async function discharge() {
    setSaving(true);
    const supabase = createClient();
    await supabase.from("inpatients").update({ status:"discharged", discharge_date: today }).eq("id", inpatientId);
    setSaving(false); setPanel("none"); router.refresh();
  }
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
          {isActive && !todayVisitId && (
            <button onClick={() => setPanel("visit")}
              style={{ background:"#3b82f6", color:"#fff", border:"none", borderRadius:"14px", padding:"18px", fontSize:"16px", fontWeight:"700", cursor:"pointer", fontFamily:"inherit" }}>
              + Record Today&apos;s Visit
            </button>
          )}
          {todayVisitId && (
            <button onClick={() => router.push(`/doctor/visit/${todayVisitId}`)}
              style={{ background:"#1d4ed8", color:"#93c5fd", border:"none", borderRadius:"14px", padding:"16px", fontSize:"15px", fontWeight:"700", cursor:"pointer", fontFamily:"inherit" }}>
      ...
      {panel === "visit" && (
        <div style={{ background:"#1e293b", borderRadius:"16px", padding:"16px", marginBottom:"4px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"14px" }}>
            <div style={{ fontSize:"14px", fontWeight:"700" }}>New Visit — {today}</div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"6px", marginBottom:"12px" }}>
            {VISIT_TYPES.map(vt => (
              <button key={vt.key} onClick={() => setVisitType(vt.key)}
                style={{ background: visitType===vt.key ? "#3b82f6" : "#0f172a", color: visitType===vt.key ? "#fff" : "#64748b",
                  border: `1.5px solid ${visitType===vt.key?"#3b82f6":"#334155"}`, borderRadius:"10px",
                  padding:"10px", fontSize:"13px", fontWeight:"600", cursor:"pointer", fontFamily:"inherit" }}>
                {vt.label}
              </button>
            ))}
          </div>
          <input type="number" value={visitFee} onChange={e => setVisitFee(e.target.value)}
            ...
            style={{ width:"100%", background:"#0f172a", border:"1.5px solid #334155", borderRadius:"10px", color:"#f1f5f9", padding:"12px 16px", fontSize:"16px", fontFamily:"monospace", marginBottom:"10px", boxSizing:"border-box" }} />
          <textarea value={visitNote} onChange={e => setVisitNote(e.target.value)} rows={2}
            placeholder="Quick note (optional — you can add full notes after saving)"
            style={{ width:"100%", background:"#0f172a", border:"1.5px solid #334155", borderRadius:"10px", color:"#f1f5f9", padding:"12px", fontSize:"14px", resize:"none", fontFamily:"inherit", marginBottom:"10px", boxSizing:"border-box" }} />
          {error && <div style={{ background:"#450a0a", border:"1px solid #dc2626", borderRadius:"8px", padding:"8px 12px", color:"#fca5a5", fontSize:"12px", marginBottom:"10px" }}>⚠ {error}</div>}
      ...
      {panel === "location" && (
        <div style={{ background:"#1e293b", borderRadius:"16px", padding:"16px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"12px" }}>
            <div style={{ fontSize:"14px", fontWeight:"700" }}>Update Room / Location</div>
            <button onClick={() => setPanel("none")} style={{ background:"none", border:"none", color:"#64748b", fontSize:"18px", cursor:"pointer" }}>✕</button>
          </div>
      ...
      {panel === "discharge" && (
        <div style={{ background:"#1e293b", border:"1.5px solid #7f1d1d", borderRadius:"16px", padding:"16px" }}>
          ...
          <div style={{ fontSize:"13px", color:"#94a3b8", marginBottom:"14px" }}>
            This will mark the patient as discharged today ({today}). This action can be reviewed in the full inpatient file.
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px" }}>
            ...
            <button onClick={discharge} disabled={saving}
